...
public class Rules {
    ...
    private Vector wallPieceMoves( int piecePosition, boolean jump, int pieceType , Player aPlayer ) {
		WallPieceState state;
		if (pieceType == Board.KING) {
			state = new WallPieceStateKing(theBoard, leftWallPieces, rightWallPieces, piecePosition, jump, aPlayer);
		} else if (aPlayer.getColor() == Color.white) {
			state = new WallPieceStateWhite(theBoard, leftWallPieces, rightWallPieces, piecePosition, jump, aPlayer);
		} else {
			state = new WallPieceStateBlue(theBoard, leftWallPieces, rightWallPieces, piecePosition, jump, aPlayer);
		}
		state.behavior();
